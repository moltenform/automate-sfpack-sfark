...
def runTest():
    ...
    startScript(
        lambda: 0, unpackSfpack, getFilenamesAndCheckIfFilesAlreadyExist, runTest, '.sfpack', files.getName(__file__)
    )
...
def checkPrereqsBeforeRun():
    ...
    trace(
        "if a message appears saying 'Could not register shell extension!', it's not a problem, just wait a few seconds and we will close it."
    )
...
def go():
    startScript(
        checkPrereqsBeforeRun, unpackSfpack, getFilenamesAndCheckIfFilesAlreadyExist, runTest, '.sfpack',
        files.getName(__file__)
    )
